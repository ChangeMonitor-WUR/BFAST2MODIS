...
Please go to the [MODIS Land subsets website](http://daac.ornl.gov/cgi-bin/MODIS/GR_col5_1/mod_viz.html):
  ...
  * Via this site the data can be downloaded manually. In the tutorial we will automatically download MODIS data using R scripting, as explained below.
...
The necessary add-on packages need to be installed within R before loading them in `R` using the `library()` function. Below we define a helper function that does installing and loading of the packages for us. 
